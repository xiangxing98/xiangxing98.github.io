...
geom:
including:point, bar, boxplot, line

scaling:
基本上是把我们的数据转化为电脑可识别的数据
比如将坐标映射到[0, 1]的一个数 将颜色映射到三维颜色空间中的一个点 将形状映射为一个整数

一个图包括三个部分:
1) data: 也就是geom
2) scales and coordinate system: 包括坐标轴和图例
3) plot annotations: 背景和标题

Scaling分为三个部分: transforming, training, mapping
